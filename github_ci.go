...
func (f *gitHubForge) CI() CIService {
	return &gitHubCIService{client: f.client}
}

func convertGitHubWorkflowRun(r *github.WorkflowRun) CIRun {
	result := CIRun{
		ID:     r.GetID(),
		Title:  r.GetName(),
		Status: r.GetStatus(),
...
func (s *gitHubCIService) ListRuns(ctx context.Context, owner, repo string, opts ListCIRunOpts) ([]CIRun, error) {
...
func (s *gitHubCIService) GetRun(ctx context.Context, owner, repo string, runID int64) (*CIRun, error) {
...
func (s *gitHubCIService) TriggerRun(ctx context.Context, owner, repo string, opts TriggerCIRunOpts) error {
