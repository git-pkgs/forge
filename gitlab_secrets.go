...
func (f *gitLabForge) Secrets() SecretService {
	return &gitLabSecretService{client: f.client}
}

func (s *gitLabSecretService) List(ctx context.Context, owner, repo string, opts ListSecretOpts) ([]Secret, error) {
	pid := owner + "/" + repo
...
func (s *gitLabSecretService) Set(ctx context.Context, owner, repo string, opts SetSecretOpts) error {
	pid := owner + "/" + repo
