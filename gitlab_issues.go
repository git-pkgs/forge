...
func (f *gitLabForge) Issues() IssueService {
	return &gitLabIssueService{client: f.client}
}

func convertGitLabIssue(i *gitlab.Issue) Issue {
	result := Issue{
		Number:  int(i.IID),
		Title:   i.Title,
		Body:    i.Description,
...
func convertGitLabNote(n *gitlab.Note) Comment {
	result := Comment{
		ID:   n.ID,
		Body: n.Body,
		Author: User{
...
func (s *gitLabIssueService) Get(ctx context.Context, owner, repo string, number int) (*Issue, error) {
	pid := owner + "/" + repo
...
func (s *gitLabIssueService) List(ctx context.Context, owner, repo string, opts ListIssueOpts) ([]Issue, error) {
	pid := owner + "/" + repo
...
func (s *gitLabIssueService) Create(ctx context.Context, owner, repo string, opts CreateIssueOpts) (*Issue, error) {
	pid := owner + "/" + repo
...
func (s *gitLabIssueService) Update(ctx context.Context, owner, repo string, number int, opts UpdateIssueOpts) (*Issue, error) {
	pid := owner + "/" + repo
...
func (s *gitLabIssueService) CreateComment(ctx context.Context, owner, repo string, number int, body string) (*Comment, error) {
	pid := owner + "/" + repo
...
func (s *gitLabIssueService) ListComments(ctx context.Context, owner, repo string, number int) ([]Comment, error) {
	pid := owner + "/" + repo
	var all []Comment
