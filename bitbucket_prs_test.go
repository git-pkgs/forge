...
func TestBitbucketListPRs(t *testing.T) {
	...
	s := &bitbucketPRService{httpClient: srv.Client()}
	prs, err := s.List(context.Background(), "atlassian", "stash", ListPROpts{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(prs) != 2 {
...
func TestBitbucketCreatePR(t *testing.T) {
	...
	s := &bitbucketPRService{httpClient: srv.Client()}
	pr, err := s.Create(context.Background(), "atlassian", "stash", CreatePROpts{
		Title: "New PR",
		Body:  "Description",
		Head:  "feature",
		Base:  "main",
...
func TestBitbucketMergePR(t *testing.T) {
	...
	s := &bitbucketPRService{httpClient: srv.Client()}
	if err := s.Merge(context.Background(), "atlassian", "stash", 1, MergePROpts{Method: "merge_commit"}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}
