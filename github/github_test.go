...
func TestGitHubListRepos(t *testing.T) {
	...
	s := newTestGitHubRepoService(srv)
	repos, err := s.List(context.Background(), "myorg", forge.ListRepoOpts{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(repos) != 2 {
...
func TestGitHubListReposFallbackToUser(t *testing.T) {
	...
	s := newTestGitHubRepoService(srv)
	repos, err := s.List(context.Background(), "someuser", forge.ListRepoOpts{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(repos) != 1 {
...
func TestGitHubListReposWithFilters(t *testing.T) {
	...
	s := newTestGitHubRepoService(srv)
	repos, err := s.List(context.Background(), "myorg", forge.ListRepoOpts{
		Archived: forge.ArchivedExclude,
		Forks:    forge.ForkExclude,
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestGitHubCreateRepo(t *testing.T) {
	...
	s := newTestGitHubRepoService(srv)
	repo, err := s.Create(context.Background(), forge.CreateRepoOpts{
		Name:        "new-repo",
