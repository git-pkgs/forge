...
func TestGitHubDeployKeyList(t *testing.T) {
	...
	keys, err := svc.List(context.Background(), "octocat", "hello", forge.ListDeployKeyOpts{})
	if err != nil {
...
func TestGitHubDeployKeyCreate(t *testing.T) {
	...
	svc := newTestGitHubDeployKeyService(srv)
	key, err := svc.Create(context.Background(), "octocat", "hello", forge.CreateDeployKeyOpts{
		Title:    "new-key",
		Key:      "ssh-rsa BBBB...",
		ReadOnly: true,
	})
