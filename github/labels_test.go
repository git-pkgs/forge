...
func TestGitHubListLabels(t *testing.T) {
	...
	labels, err := s.List(context.Background(), "octocat", "hello-world", forge.ListLabelOpts{})
	if err != nil {
...
func TestGitHubCreateLabel(t *testing.T) {
	...
	s := newTestGitHubLabelService(srv)
	label, err := s.Create(context.Background(), "octocat", "hello-world", forge.CreateLabelOpts{
		Name:  "priority",
		Color: "ff0000",
	})
	if err != nil {
...
func TestGitHubUpdateLabel(t *testing.T) {
	...
	s := newTestGitHubLabelService(srv)
	newColor := "00ff00"
	label, err := s.Update(context.Background(), "octocat", "hello-world", "bug", forge.UpdateLabelOpts{
		Color: &newColor,
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
