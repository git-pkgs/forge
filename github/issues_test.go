...
func TestGitHubListIssues(t *testing.T) {
	...
	s := newTestGitHubIssueService(srv)
	issues, err := s.List(context.Background(), "octocat", "hello-world", forge.ListIssueOpts{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(issues) != 2 {
...
func TestGitHubCreateIssue(t *testing.T) {
	...
	s := newTestGitHubIssueService(srv)
	issue, err := s.Create(context.Background(), "octocat", "hello-world", forge.CreateIssueOpts{
		Title: "New bug",
		Body:  "Details here",
	})
	if err != nil {
