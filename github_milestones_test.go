...
func TestGitHubListMilestones(t *testing.T) {
	...
	milestones, err := s.List(context.Background(), "octocat", "hello-world", ListMilestoneOpts{})
	if err != nil {
...
func TestGitHubCreateMilestone(t *testing.T) {
	...
	s := newTestGitHubMilestoneService(srv)
	milestone, err := s.Create(context.Background(), "octocat", "hello-world", CreateMilestoneOpts{
		Title:       "v3.0",
