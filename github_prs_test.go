...
func TestGitHubListPRs(t *testing.T) {
	...
	s := newTestGitHubPRService(srv)
	prs, err := s.List(context.Background(), "octocat", "hello-world", ListPROpts{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(prs) != 2 {
...
func TestGitHubCreatePR(t *testing.T) {
	...
	s := newTestGitHubPRService(srv)
	pr, err := s.Create(context.Background(), "octocat", "hello-world", CreatePROpts{
		Title: "New feature",
		Body:  "Description",
		Head:  "my-branch",
		Base:  "main",
...
func TestGitHubMergePR(t *testing.T) {
	...
	s := newTestGitHubPRService(srv)
	if err := s.Merge(context.Background(), "octocat", "hello-world", 1, MergePROpts{Method: "squash"}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}
