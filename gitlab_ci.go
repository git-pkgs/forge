...
func (f *gitLabForge) CI() CIService {
	return &gitLabCIService{client: f.client}
}

func convertGitLabPipeline(p *gitlab.PipelineInfo) CIRun {
	result := CIRun{
		ID:     int64(p.ID),
		Status: p.Status,
		Branch: p.Ref,
...
func convertGitLabPipelineDetail(p *gitlab.Pipeline) CIRun {
	result := CIRun{
		ID:      int64(p.ID),
		Status:  p.Status,
		Branch:  p.Ref,
...
func (s *gitLabCIService) ListRuns(ctx context.Context, owner, repo string, opts ListCIRunOpts) ([]CIRun, error) {
	pid := owner + "/" + repo
...
func (s *gitLabCIService) GetRun(ctx context.Context, owner, repo string, runID int64) (*CIRun, error) {
	pid := owner + "/" + repo
...
func (s *gitLabCIService) TriggerRun(ctx context.Context, owner, repo string, opts TriggerCIRunOpts) error {
	pid := owner + "/" + repo
